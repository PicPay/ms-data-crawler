...
var (
	...
	IncompatibleServiceResponse = errors.New("json: cannot unmarshal object into Go value of type []map[string]string")
)
...
func (s *ServiceSuite) TestIncompatibleServiceResponse() {
	ctx := context.Background()
	s.mockGateway.EXPECT().Find(ctx, gomock.Any()).Return(getMockedConfigurationWithoutResponseFormat(), nil).Times(1)
	getSource := s.mockHttpClient.EXPECT().Send(gomock.Any(), gomock.Any(), gomock.Any()).Return(getHttpSourceResponse(), nil).Times(1)
	...
	_, err := s.service.Fetch(ctx, identifier, headers)

	s.Error(IncompatibleServiceResponse, err)
}

func (s *ServiceSuite) TestFetchEmptyDataConfiguration() {
	ctx := context.Background()
	s.mockGateway.EXPECT().Find(ctx, gomock.Any()).Return(getMockedConfiguration(), nil).Times(1)
	getSource := s.mockHttpClient.EXPECT().Send(gomock.Any(), gomock.Any(), gomock.Any()).Return(getHttpSourceBadRequestResponse(), nil).Times(1)
	...
	s.NoError(err)
	expectedResponse := getEmptyConfigurationResponse()

	s.Equal(expectedResponse, actualResponse)
}

func (s *ServiceSuite) TestFetchErrorDataConfiguration() {
	ctx := context.Background()
	s.mockGateway.EXPECT().Find(ctx, gomock.Any()).Return(getMockedConfiguration(), nil).Times(1)
	getSource := s.mockHttpClient.EXPECT().Send(gomock.Any(), gomock.Any(), gomock.Any()).Return(getHttpSourceInternalServerErrorResponse(), nil).Times(1)
...
func (s *ServiceSuite) TestConfigurationNotFound() {
	ctx := context.Background()
	s.mockGateway.EXPECT().Find(ctx, gomock.Any()).Return(getNotFoundConfiguration()).Times(1)

	_, err := s.service.Fetch(ctx, identifier, headers)

	s.Error(ConfigNotFound, err)
}

func getNotFoundConfiguration() (*Configuration, error) {
...
func getMockedConfigurationWithoutResponseFormat() *Configuration {
	...
	var configurationMocked = Configuration{
		ID:         id,
		Identifier: identifier,
		Source:     getSourceWithoutResponseFormat(),
		Crawler:    getCrawler(),
		CreatedAt:  dateMock,
		UpdatedAt:  dateMock,
	}
...
func getHttpSourceBadRequestResponse() v1.HttpResponse {
	response := "{\"_id\":\"991234\",\"consumer_id\":\"7620665\",\"items\":[]}"
	return v1.HttpResponse{
		Status: BadRequest,
		Body:   []byte(response),
	}
...
func getHttpSourceInternalServerErrorResponse() v1.HttpResponse {
	response := "<html><head><title>Oops</title></head><body>The server went ballons</body></html>"
	return v1.HttpResponse{
...
func getSourceWithoutResponseFormat() ServiceRequest {
	return ServiceRequest{
		Name:        "midgard",
		Url:         "http://midgard.ms.prod/v1/collections/store_recently_viewed/<Consumer_Id>",
		Method:      "GET",
...
func getEmptyConfigurationResponse() *AssembledScreen {
	return &AssembledScreen{
		Identifier: identifier,
		Data:       []interface{}(nil),
		CreatedAt:  dateMock,
